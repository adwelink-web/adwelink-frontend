...
import { getInstituteSettings, updateInstituteSettings } from "./actions"

export default function InstituteSettingsPage() {
    const [originalSettings, setOriginalSettings] = React.useState<any>(null)
                ...
                <Tabs defaultValue="general" className="flex-1 flex flex-col min-h-0">
                    <div className="flex-none space-y-6 mb-8">
                        <div className="flex flex-col gap-1">
                            ...
                            <p className="text-slate-400">Mission Control for your Organization. Manage everything from Branding to Billing.</p>
                        </div>
                                ...
                                <Card className="bg-gradient-to-b from-amber-500/10 to-transparent border-amber-500/20">
                                    <CardHeader>
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className="bg-amber-500/20 text-amber-500 text-xs px-2 py-1 rounded capitalize">{settings?.subscription_status || "Active"}</span>
                                        </div>
                                        <CardDescription className="text-amber-200/60">Managed via Adwelink Billing Portal</CardDescription>
                                    </CardHeader>
                                    <CardContent className="space-y-6">
                                        <div className="text-3xl font-bold text-white">₹2,999<span className="text-sm text-slate-400 font-normal">/mo</span></div>
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2 text-sm text-slate-300"><Check className="h-4 w-4 text-emerald-500" /> All AI Agents Enabled</div>
                                            <div className="flex items-center gap-2 text-sm text-slate-300"><Check className="h-4 w-4 text-emerald-500" /> Unlimited Lead Processing</div>
                                        </div>
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="flex items-center justify-between p-3 border border-white/10 rounded bg-white/5">
                                            <div className="flex items-center gap-3">
                                                <div className="h-8 w-12 bg-slate-700 rounded flex items-center justify-center text-xs text-white uppercase">{settings?.payment_method_type || "UPI/CARD"}</div>
                                                <span className="text-white text-sm">Linked Account</span>
                                            </div>
                                            <Button variant="ghost" size="sm" className="text-slate-400">View</Button>
                                        </div>
                                        <Button variant="outline" className="w-full border-white/10 text-slate-300 hover:bg-white/5">+ Manage Billing</Button>
                                    </CardContent>
                                </Card>
                            </div>
                        </TabsContent>
